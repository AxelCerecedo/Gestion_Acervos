document.getElementById("login-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (data.usuario.rol === "lector") {
      ...
      sessionStorage.setItem("seccionesPermitidas", JSON.stringify(secciones));
      window.location.href = "analytics.html";
    } 
// 👇 REGLA PARA CIBERSEGURIDAD (en login.js)
    else if (data.usuario.rol === "ciberseguridad") {
      const secciones = ["cyberseguridad", "perfil"];
      sessionStorage.setItem("seccionesPermitidas", JSON.stringify(secciones));
      window.location.href = "cyberseguridad.html"; 
    }
    // 👇 ADMIN (Ve todo)
    else {
